...
/**
 * Extracts a [LeafInfo] from a single leaf descriptor for sidebar rendering.
 *
 * @param leaf a dynamic leaf descriptor from the server config
 * @param into the mutable list to append a [LeafInfo] entry to
 */
fun collectLeaves(leaf: dynamic, into: MutableList<LeafInfo>) {
    val kind = (leaf.content?.kind as? String) ?: "terminal"
    val link = leaf.isLink as? Boolean ?: false
    into.add(LeafInfo(leaf.id as String, leaf.title as String, leaf.sessionId as String, kind, link))
}
...
fun renderSidebar(config: dynamic) {
    ...
    for (tab in tabsArr) {
        ...
        for (p in panes) collectLeaves(p.leaf, leaves)
        ...
        for ((paneId, paneTitle, sId, contentKind, isLinked) in leaves) {
            ...
            item.addEventListener("click", { ev ->
                ...
                fun doFocus() {
                    ...
                    // Scope to `.terminal-cell` — the outer `.floating-pane`
                    // also carries `data-pane`, and an unscoped selector
                    // returns the wrapper first, which isn't the element
                    // `markPaneFocused` expects.
                    val cellEl = wrap.querySelector(".terminal-cell[data-pane=\"$paneId\"]") as? HTMLElement
